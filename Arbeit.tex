...
konstante Magnetfeld $B_z$ aufgehoben. Das Wechselfeld $\vec B_\text{T}$ zeige in
$x'$-Richtung. Analog zur vorherigen Präzession um die $z$-Achse, stellt sich nun eine Präzession um die $x'$-Achse ein. Die Winkelgeschwindigkeit beträgt hier
\[
...
\begin{figure}
    ...
    \begin{tikzpicture}
        % x'-Achse
        \draw[thick,->] (0,0) to (-2.5,-2.5) node[left]{$x'$};
        % y'-Achse
        \draw[thick,->] (0,0) to (5,0) node[right]{$y'$};
        % z'-Achse
        \draw[thick,->] (0,0) to (0,5) node[right]{$z$};
        % Wechselfeld
        \draw[ultra thick, dashed, ->] (0,0) to (-1.5,-1.5) node[label=0:{$\vec B_\text{T}$}]{};
        % Magnetisierungen
        \draw[ultra thick, ->] (0,0) to (0,3.5) node[left]{$M(t=0)$};
        \draw[ultra thick, ->] (0,0) to (3.5,0) node[below]{$M\del{t_{\frac\piup2}}$};

        \draw (3.5,0) arc (0:90:3.5);
        \draw (2.5,2.5) -- (3.2,3.2) node[right]{$M(t)$};
    \end{tikzpicture}
    \caption{%
